...
class KBM(dynamics_model_base):
    ...
    def dynamics(self, state: np.ndarray, action: np.ndarray) -> np.ndarray:
        ...
        # Split columns
        _, _, theta = torch.hsplit(state, 3)
        v, omega = torch.hsplit(action, 2)

        # Enforce constraints
        v = torch.clip(v, self.min_throttle, self.max_throttle)
        omega = torch.clip(omega, -self.max_steer, self.max_steer)

        # Perform update
        x_dot = v * torch.cos(theta)
        y_dot = v * torch.sin(theta)
        theta_dot = v * torch.tan(omega) / dynamics_model_base.L

        new_state = torch.concatenate([x_dot, y_dot, theta_dot], dim=1)
